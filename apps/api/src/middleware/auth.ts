...
export const clerkAuth: MiddlewareHandler<{
  ...
  if (!secretKey) {
    ...
    return c.json(
      { error: "Internal server error: auth not configured" },
      500
    );
  }
...
// ─── Org Access Guard ────────────────────────────────────
//
// Must be used after clerkAuth. Resolves the org identifier from
// a path param (or query param), verifies the authenticated user
// is an OrgMember of that org, and returns 403 otherwise.

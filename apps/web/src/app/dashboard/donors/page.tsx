...
// Placeholder org ID — replaced by real org context in production
...
export default function DonorsPage() {
  ...
  async function handleExportCsv() {
    ...
    try {
      ...
      const params = new URLSearchParams({ orgId: PLACEHOLDER_ORG_ID });
      if (search) params.set("search", search);

      const res = await fetch(
        `${API_BASE}/api/donors/export?${params.toString()}`,
        {
          headers: {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={exporting}
            className="flex-shrink-0 inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-gray-200 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {exporting ? (
              <>
                <svg

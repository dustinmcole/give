...
**NPC object model:** Different from NPSP. `GiftTransaction` (not Opportunity), `GiftCommitment` (not npe03__), `GiftTransactionDesignation`, `DonorGiftSummary`. Person Accounts (not Contact + HH Account). Business Process API (BPAPI) available for third-party integrators. NPC adoption still < NPSP but both supported in v1 (auto-detected at setup). Use BPAPI for NPC syncs.
